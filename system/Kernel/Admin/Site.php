...
class Admin_Site extends Site
{
	...
	public function action($uri_path)
	{
		...
		$this->output_data['menu'] = array(
			...
			'meta' => array(
				'title' => 'Мета-данные',
				'link' => $this->EE->breadcrumbs[count($this->EE->breadcrumbs) - 1]['uri'] . 'meta/',
				),
			'domains' => array(
				'title' => 'Домены',
				'link' => $this->EE->breadcrumbs[count($this->EE->breadcrumbs) - 1]['uri'] . 'domains/',
				),
			);
		...
		switch ($uri_path_parts[0]) {
			case '':
				$this->output_data['site_edit_form'] = $this->getSiteEditFormData();
				$this->output_data['menu']['site']['selected'] = true;
				break;
			case 'meta':
				$this->EE->addBreadCrumb('', 'Мета-данные');
				$Meta = new Component_Meta($this->getMetaData());
				$this->output_data['meta_controls'] = $Meta->getControls();
				$this->output_data['menu']['meta']['selected'] = true;
				break;
			case 'domains':
				$this->output_data['domains'] = $this->getDomainsList();
				$this->output_data['menu']['domains']['selected'] = true;
				$this->EE->addBreadCrumb('', 'Домены');
				break;
			default;
		}
	}
