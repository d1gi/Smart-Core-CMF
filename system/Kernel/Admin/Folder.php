...
class Admin_Folder extends Folder
{
	...
	public function action($uri_path)
	{
		...
		if (is_numeric($uri_path_parts[0])) {
			$this->EE->addBreadCrumb($uri_path_parts[0] . '/', 'Редактирование папки id: ' . $uri_path_parts[0]);
			...
			if (isset($uri_path_parts[1]) and $uri_path_parts[1] === 'meta') {
				$this->EE->addBreadCrumb('', 'Мета-данные');
				$Meta = new Component_Meta(unserialize($folder->meta));
				$this->output_data['meta_controls'] = $Meta->getControls(array('folder_id' => $uri_path_parts[0]));
			} else {
				$Node = new Node();
				$this->output_data['meta']  = unserialize($folder->meta);
				$this->output_data['nodes'] = $Node->getListInFolder($uri_path_parts[0]);
				$this->output_data['form']  = $this->getEditFormData($uri_path_parts[0]);
			}
		}
		// Создание папки
		else if ($uri_path_parts[0] == 'create') {
			$this->EE->addBreadCrumb('', 'Создание новой папки');
		...
		// Список всех папок
			...
			$this->output_data['all'] = $this->getList();
		}
	}
	...
	public function postProcessor()
	{
		...
		switch ($submit) {
			...
			case 'create_meta':
				...
				cf_redirect($this->EE->breadcrumbs[count($this->EE->breadcrumbs) - 1]['uri'] . $folder_id . '/meta/');
				break;
		...
		if ($_POST['target'] == '_parent') {
			if ($_POST['pd']['is_active']) {
				cf_redirect(parent::getUri($folder_id));
			} else {
				cf_redirect(parent::getUri(1));
			}
		} else {
