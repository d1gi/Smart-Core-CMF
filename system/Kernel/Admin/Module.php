 ...
 */
class Admin_Module extends Base
{
	...
	{
		if (empty($uri_path)) {
			$this->output_data['modules'] = $this->getModulesList();
		} else {
			$uri_path_parts = explode('/', $uri_path);
			if ($this->isModuleExist($uri_path_parts[0])) {
				$this->EE->addBreadCrumb($uri_path_parts[0] . '/', $uri_path_parts[0]);
				$module_class_name	= 'Module_' . $uri_path_parts[0] . '_Admin';
				$Module				= new $module_class_name(false);
				$this->module_name	= $uri_path_parts[0];
				$this->output_data	= $Module->admin(substr($uri_path, strlen($uri_path_parts[0]) + 1));
				$this->setTpl($Module->getTpl());
				$this->setTplPath(DIR_MODULES . $uri_path_parts[0] . '/');
			}
		}
	}
