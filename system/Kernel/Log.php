 ...
 * @uses 		User
 */	
class Log extends SingletonBase
{
	private $user_id;
	private $user_login;
		...
		switch ($type) {
			case 'http_error':
				$sql = "
					...
					VALUES
						('$user_id', '{$this->Env->site_id}', $message, '$datetime', $requested_uri, $ip, $referer, $user_agent, '$browser', '$browser_version', '$platform') ";
				$this->DB->exec($sql);
				break;
			case 'user_auth':
				$sql = "
					...
					VALUES
						('$user_id', '{$this->Env->site_id}', '$login', $message, $referer, '$datetime', $ip, $user_agent, '$browser', '$browser_version', '$platform') ";
				$this->DB->exec($sql);
				break;
			case 'system':
				$hash = md5($message);
				...
				if ($result->rowCount() == 0) {
					$sql = "
						...
						VALUES
							('$user_id', '{$this->Env->site_id}', $message, '$hash', '$datetime', $requested_uri, $ip, $referer, $user_agent, '$browser', '$browser_version', '$platform') ";
					$this->DB->exec($sql);
				}
				break;
			default;
