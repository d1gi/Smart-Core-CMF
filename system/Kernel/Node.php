...
class Node extends Base
{
	 ...
	 * @return void
	 */
	private function __SAMPLE_USE()
	{
		// Создание объекта Ноды.
		$Node = new Node();
		
		// Создание объекта Ноды и сразу активировать указанную ноду.
		$Node = new Node($node_id);
		
		// Использование (Активирование ноды).
		$Node->activate($node_id);
		
		// Паттерн фабрика
		// Получить объект модуля 
		$Module = $Node->getModuleInstance($node_id);
		
		$Node->create($pd);
		$Node->update($pd, $node_id = false);
		$Node->getProperties($node_id);
		$Node->getCreateFormData($node_id);
		$Node->getEditFormData($node_id);
		$Node->getListInFolder($folder_id);
		
		/**
		 * Запросить некоторые данные у ноды (хуки).
		 * 
		 * Примеры использования:
		 * - Модуль комментариев будет запрашивать у модуля новостей(каталога) ИД записи, и основываясь на этом ИД генерировать список комментариев.
		 * - Модуль меню будет запрашивать у модуля комментариев дерево категорий. Притом надо учесть дополнительные параметры, например:
		 *   - ИД категории которую надо вывести.
		 *   - Глубину вложенности категорий.
		 */
		
		// Параметры это сериализованный массив с данными, например:
		$params = array(
