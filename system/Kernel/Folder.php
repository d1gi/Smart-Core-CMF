...
class Folder extends Base
{
	...
	public function getDataById($folder_id, $language = false)
	{
		...
		return $this->DB->getRowObject($sql);
	}
	...
	public function create($pd)
	{
		...
		$sql = "SELECT max(folder_id) AS folder_id
			FROM {$this->DB->prefix()}engine_folders
			WHERE site_id = '{$this->Env->site_id}' ";
		$folder_id = $this->DB->getRowObject($sql)->folder_id + 1;
		...
		$sql = "
			...
			VALUES 
				('$folder_id', '{$this->Env->site_id}', '{$this->Env->language_id}', $title, $descr) ";
		$this->DB->exec($sql);
		*/
	...
	{ 
		...
		if ($result->rowCount() > 0) {
			...
			while ($row = $result->fetchObject()) {
				...
				if ($max_depth != false && $max_depth < $this->_tree_level) { // копаем до указанной глубины.
					continue;
				}
	...
	/**
	 * Получить полную ссылку на папку, указав её id. Если не указать ид папки, то вернуть текущий путь.
	 * 
	 * @param int $folder_id
	 * @return string $uri
	 */
